...
/*! IARU add qso
     ...
     [1]==5 stations in different country but same continent, HQ or IARU
   officials [2]=10 stations in different continents
 */
...
int IARU::numberField() const { return -1; }
...
bool IARU::validateExchange(Qso *qso) {
  ...
  if (qso->isamult[1]) {
    ...
    if (ok && qso->zone > 0 && qso->zone <= zoneMax()) {
      qso->isamult[0] = true;
      qso->mult[0] =
          qso->zone - 1; // important: update zone with what was logged!
    } else {
      qso->isamult[0] = false;
      qso->zone = 0;
      qso->mult[0] = -1;
