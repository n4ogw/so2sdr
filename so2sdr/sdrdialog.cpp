...
void SDRDialog::fileGetter(QString msg, QString path, QString files,
                           QString key, QLabel *label) {
  QString fileName = QFileDialog::getSaveFileName(
      this, msg, path, files, nullptr, QFileDialog::DontConfirmOverwrite);
  if (fileName.isNull()) {
    return;
  }
...
void SDRDialog::findConfig1() {
  fileGetter("So2sdr-bandmap config file",
             userDirectory() + "/so2sdr-bandmap1.ini", "ini files (*.ini)",
             s_sdr_config[0], labelConfig1);
}

void SDRDialog::findConfig2() {
  fileGetter("So2sdr-bandmap config file",
             userDirectory() + "/so2sdr-bandmap2.ini", "ini files (*.ini)",
             s_sdr_config[1], labelConfig2);
}

void SDRDialog::findExeFile1() {
  fileGetter("So2sdr-bandmap executable",
             QCoreApplication::applicationDirPath(), "so2sdr-bandmap",
             s_sdr_path[0], labelExe1);
}

void SDRDialog::findExeFile2() {
  fileGetter("So2sdr-bandmap executable",
             QCoreApplication::applicationDirPath(), "so2sdr-bandmap",
             s_sdr_path[1], labelExe2);
}

void SDRDialog::updateFromSettings() {
  SpotTimeoutLineEdit->setText(
      settings.value(s_sdr_spottime, s_sdr_spottime_def).toString());
    ...
    pathLabel[i]->setText(
        settings
            .value(s_sdr_path[i],
                   QCoreApplication::applicationDirPath() + "so2sdr-bandmap")
            .toString());
    configLabel[i]->setText(shortName(
        settings.value(s_sdr_config[i], s_sdr_config_def[i]).toString()));
    ipPtr[i]->setText(settings.value(s_sdr_ip[i], s_sdr_ip_def[i]).toString());
    portPtr[i]->setText(
        settings.value(s_sdr_port[i], s_sdr_port_def[i]).toString());
  }
  ChangeRadioClickCheckBox->setChecked(
      settings.value(s_sdr_changeclick, s_sdr_changeclick_def).toBool());
  lineEdit160low->setText(
      settings.value(s_sdr_cqlimit_low[0], cqlimit_default_low[0]).toString());
  lineEdit160high->setText(
      settings.value(s_sdr_cqlimit_high[0], cqlimit_default_high[0])
          .toString());
  lineEdit80low->setText(
      settings.value(s_sdr_cqlimit_low[1], cqlimit_default_low[1]).toString());
  lineEdit80high->setText(
      settings.value(s_sdr_cqlimit_high[1], cqlimit_default_high[1])
          .toString());
  lineEdit40low->setText(
      settings.value(s_sdr_cqlimit_low[2], cqlimit_default_low[2]).toString());
  lineEdit40high->setText(
      settings.value(s_sdr_cqlimit_high[2], cqlimit_default_high[2])
          .toString());
  lineEdit20low->setText(
      settings.value(s_sdr_cqlimit_low[3], cqlimit_default_low[3]).toString());
  lineEdit20high->setText(
      settings.value(s_sdr_cqlimit_high[3], cqlimit_default_high[3])
          .toString());
  lineEdit15low->setText(
      settings.value(s_sdr_cqlimit_low[4], cqlimit_default_low[4]).toString());
  lineEdit15high->setText(
      settings.value(s_sdr_cqlimit_high[4], cqlimit_default_high[4])
          .toString());
  lineEdit10low->setText(
      settings.value(s_sdr_cqlimit_low[5], cqlimit_default_low[5]).toString());
  lineEdit10high->setText(
      settings.value(s_sdr_cqlimit_high[5], cqlimit_default_high[5])
          .toString());
}
