/*! Copyright 2010-2025 rec. Torsten Clay N4OGW
...
void DetailedEdit::loadRecord(const QSqlRecord &r, int nexchange) {
  ...
  freqLineEdit->setText(
      QString::number(rec.value(SQL_COL_FREQ).toDouble(), 'f', 0));
  if (nexchange < 4) {
...
void DetailedEdit::updateRecord() {
  ...
  if (dateTimeEdit->date().toString("MMddyyyy") !=
      rec.value(SQL_COL_DATE).toString()) {
    rec.setValue(SQL_COL_DATE,
                 QVariant(dateTimeEdit->date().toString("MMddyyyy")));
    rec.setGenerated(SQL_COL_DATE, true);
  }
  if (dateTimeEdit->time().toString("hhmm") !=
      rec.value(SQL_COL_TIME).toString()) {
