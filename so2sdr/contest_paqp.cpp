...
QList<QString> PAQP::EPA_counties = QList<QString>() << "ADA"
                                                     << "BER"
                                                     << "BRA"
                                                     << "BUX"
                                                     << "CAR"
 ...
 */
int PAQP::numberField() const { return (0); }
...
bool PAQP::validateExchange(Qso *qso) {
  ...
  if (qso->isamult[0]) {
    ...
    // special case: PA stations get the "WPA" or "EPA" mult for PA county
    // worked
    if (withinState) {
      if (ok_part[1]) {
  ...
  if (!qso->isamult[1]) {
    ...
    for (int i = exchElement.size() - 1; i >= 0; i--) {
      ...
      if (exchElement.at(i) == "EPA" || exchElement.at(i) == "WPA")
        continue;
...
/*!
  ...
  */
void PAQP::setWithinState(bool b) { withinState = b; }
