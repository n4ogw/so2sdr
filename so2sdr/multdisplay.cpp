...
bool MultDisplay::eventFilter(QObject* o, QEvent* e)
{
    ...
    if (gridMode && e->type()==QEvent::MouseButtonPress) {
        ...
        if (ev->button()==Qt::RightButton) {
            ...
            connect(dialog,SIGNAL(grid(QByteArray)),this,SLOT(setCenterGrid(QByteArray)));
            dialog->show();
            return true;
            ...
            for (int i=0;i<x;i++) {
                ...
                if (upperLeft[2]==10) {
                    upperLeft[2]=0;
                    upperLeft[0]++;
                    if (upperLeft[0]=='R') upperLeft[0]='A';
                }
            }
            for (int i=0;i<y;i++) {
                upperLeft[3]--;
                if (upperLeft[3]<0) {
                    upperLeft[3]=9;
                    upperLeft[1]--;
                    if (upperLeft[1]<'A') upperLeft[1]='R';
                }
            }
            ...
            centerGrid.clear();
            centerGrid=centerGrid+centerField1+centerField2+char(centerNr1)+char(centerNr2);
            drawGrids();
            return true;
        }
    }
...
void MultDisplay::drawGrids()
{
    ...
    while (upperLeft[2]<0) {
        upperLeft[2]+=10;
        upperLeft[0]--;
        if (upperLeft[0]<'A') upperLeft[0]='R';
    }
    upperLeft[3]=(centerNr2-48)+nh/2;
    while (upperLeft[3]>=10) {
        upperLeft[3]-=10;
        upperLeft[1]++;
        if (upperLeft[1]>'R') upperLeft[1]='A';
    }
    QByteArray text;
    text.clear();
    ...
    for (int i=0;i<nh;i++) {
        ...
        for (int k=0;k<woffset;k++) text=text+"&nbsp;";
        for (int j=0;j<nw;j++) {
            QByteArray grid;
            grid.clear();
