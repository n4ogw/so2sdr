...
Contest::Contest(QSettings &cs, QSettings &ss) : settings(cs), stnSettings(ss) {
  ...
  for (int i = 0; i < MAX_EXCH_FIELDS; i++)
    multFieldHighlight[i] = -1;
  for (int i = 0; i < NModeTypes; i++)
    availableModeTypes[i] = true;
...
void Contest::initialize(const Cty *cty) {
  ...
  availableModeTypes[0] =
      settings.value(c_multimode_cw, c_multimode_cw_def).toBool();
  availableModeTypes[1] =
      settings.value(c_multimode_phone, c_multimode_phone_def).toBool();
  availableModeTypes[2] =
      settings.value(c_multimode_digital, c_multimode_digital_def).toBool();
...
QByteArray Contest::contestName() const { return (_contestName); }

QString Contest::exchangeName(int i) const { return exchName[i]; }
...
/*!
  Determine mult index. Currently only implemented for Uniques, Special, Grids,
  GridFields, and Prefix mults
...
void Contest::multIndx(Qso *qso) const {
  for (int ii = 0; ii < settings.value(c_nmulttypes, c_nmulttypes_def).toInt();
    ...
    if (qso->isamult[ii] &&
        (multType[ii] == Uniques || multType[ii] == Special ||
         multType[ii] == Prefix || multType[ii] == Grids ||
...
void Contest::addQsoMult(Qso *qso) {
  ...
  if (!qso->dupe && qso->bandColumn >= 0 && qso->bandColumn < 6)
    qsoCnt[qso->bandColumn]++;

  // mults worked counted per-band
  // last index N_BANDS is for total number of mults regardless of band
  for (int j = 0; j < settings.value(c_nmulttypes, c_nmulttypes_def).toInt();
       j++) {
    for (int k = 0; k < NModeTypes; k++) {
      for (int i = 0; i <= N_BANDS; i++) {
        multsWorked[j][k][i] = 0;
  ...
  for (int ii = 0; ii < settings.value(c_nmulttypes, c_nmulttypes_def).toInt();
    ...
    for (int k = 0; k < NModeTypes; k++) {
      if (k > 0 && !settings.value(c_multsmode, c_multsmode_def).toBool())
        break;
      for (int i = 0; i <= N_BANDS; i++) {
  ...
  for (int ii = 0; ii < settings.value(c_nmulttypes, c_nmulttypes_def).toInt();
       ii++) {
    if (_nMults[ii] == 0 || !qso->isamult[ii])
      continue;
    if (qso->mult[ii] != -1) {
      if (!multWorked[ii][mode][qso->band][qso->mult[ii]] &&
          mults[ii][qso->mult[ii]]->isamult) {
        new_bm[ii] = true;
        multsWorked[ii][mode][qso->band]++;
        multWorked[ii][mode][qso->band][qso->mult[ii]] = true;
      }
      // all-band mults
      if (!multWorked[ii][mode][N_BANDS][qso->mult[ii]] &&
          mults[ii][qso->mult[ii]]->isamult) {
        new_m[ii] = true;
...
void Contest::setGrid() {
  ...
  if (s.size() != 4 || s.at(0) < 'A' || s.at(0) > 'R' || s.at(1) < 'A' ||
      s.at(1) > 'R' || s.at(2) < '0' || s.at(2) > '9' || s.at(3) < '0' ||
      s.at(3) > '9') {
    myGrid.clear();
  } else {
    myGrid = s;
 ...
 */
void Contest::addQsoTypeCntry(int i, int ii) { qsoTypeCntry[ii].append(i); }
...
QList<QByteArray> &Contest::qsoType(int ii) { return (qsoTypeStr[ii]); }
  ...
  for (int ii = 0; ii < settings.value(c_nmulttypes, c_nmulttypes_def).toInt();
    ...
    for (int k = 0; k < NModeTypes; k++) {
      if (k > 0 && !settings.value(c_multsmode, c_multsmode_def).toBool())
        break;
      for (int i = 0; i < _nMults[ii]; i++) {
        if (multWorked[ii][k][N_BANDS].at(i))
          multsWorked[ii][k][N_BANDS]++;
      }
    }
  }
...
void Contest::workedMults(Qso *qso, unsigned int worked[MMAX]) const {
  // option 1: mults count per-band but not per-mode. Here the mult status is
  // stored internally in the CW modetype slot
  if (settings.value(c_multsband, c_multsband_def).toBool()) {
    for (int ii = 0;
         ii < settings.value(c_nmulttypes, c_nmulttypes_def).toInt(); ii++) {
      worked[ii] = 0;
    ...
    // option 2: mults count once on all bands (Sweepstakes, qso parties, etc)
    // on any mode. Here the mult status is stored in the N_BANDS slot. Note
    // that this is only set up for HF contests.
    for (int ii = 0;
         ii < settings.value(c_nmulttypes, c_nmulttypes_def).toInt(); ii++) {
      worked[ii] = 0;
      if (qso->mult[ii] != -1 && qso->mult[ii] < _nMults[ii]) {
        if (multWorked[ii][CWType][N_BANDS][qso->mult[ii]] ||
            ...
            multWorked[ii][DigiType][N_BANDS][qso->mult[ii]])
          worked[ii] = 1 + 2 + 4 + 8 + 16 + 32;
      }
    }
  }
}
...
/*!
   ...
   This may be called more than once as additional information (country,
   exchange, ...) is added to qso
...
/*! returns true if there is a default exchange that can be prefilled (zone,
 * country, etc) */
bool Contest::hasPrefill() const { return (prefill); }

/*! returns true if this contest has custom worked bits */
bool Contest::hasWorked() const { return false; }
...
bool Contest::logPrefill(int n) const {
  if (logFieldPrefill && n >= 0 && n < nExch)
    return (logFieldPrefill[n]);
  else
    return (false);
}
 ...
 */
FieldTypes Contest::exchType(int indx) const { return (exchange_type[indx]); }
...
/*! true: stations can be worked on multiple bands
    ...
    see setmultsByBand
    */
bool Contest::dupeCheckingByBand() const { return (dupeCheckingEveryBand); }
...
/*!
  ...
  just returns the number of mults in the band with the same number. Some
  contests redefine this
  */
...
/*!
  ...
  */
QByteArray Contest::neededMultName(int ii, int band, int i, bool &needed_band,
                                   bool &needed) const {
  needed = false;
  needed_band = false;
...
/*!
  ...
  */
QByteArray Contest::neededMultNameMode(int ii, int band, ModeTypes mode, int i,
 ...
 */
int Contest::nExchange() const { return (nExch); }

/*! this station's zone (CQ or ITU)
 */
int Contest::myZone() const { return (_myZone); }

/*!
   Returns number of mults of category ii
 */
int Contest::nMults(int ii) const { return (_nMults[ii]); }
...
int Contest::Score() const {
  if (settings.value(c_multsband, c_multsband_def).toBool()) {
    ...
    for (int ii = 0;
      ...
      switch (ii) {
      case 0:
        if (settings.value(c_mult1_displayonly, c_mult1_displayonly_def)
                .toBool())
          continue;
        break;
      case 1:
        if (settings.value(c_mult2_displayonly, c_mult2_displayonly_def)
                .toBool())
          continue;
        break;
      }
...
void Contest::setContinent(Cont i) { myContinent = i; }

void Contest::setCountry(int i) { myCountry = i; }

void Contest::setMyZone(int i) { _myZone = i; }
...
bool Contest::valExch_mm(Qso *qso) {
  ...
  for (int i = exchElement.size() - 1; i >= 0; i--) {
    if (exchElement[i] == "R1" || exchElement[i] == "R2" ||
        exchElement[i] == "R3") {
...
bool Contest::valExch_nr_name_state(int ii, int &mult_indx) {
  if (exchElement.size() < 3)
    return (false); // need at least 3 pieces
  ...
  // name must match the DMult list; take the next non-nr element before the
  // mult
  if (!ok[1]) {
...
bool Contest::valExch_nr_name() {
  if (exchElement.size() < 2)
    return (false); // need at least 2 pieces
  ...
  for (int i = 0; i < exchElement.size(); i++) {
    ...
    // start from 0(take first one) since some contests have third Pfx/country
    // field
    if (!ok[1] && !nr_indx[i]) {
      finalExch[1] = exchElement[i];
      ok[1] = true;
 ...
 */
int Contest::zoneMax() const { return (_zoneMax); }

void Contest::setZoneMax(int i) { _zoneMax = i; }

/*! 0 = cq zones
    1 = itu zones
 */
void Contest::setZoneType(int i) { _zoneType = i; }

int Contest::zoneType() const { return (_zoneType); }

/*! true:  contest exchange is variable (like qso number).
    false: contest exchange is fixed. Can fill cabrillo sent exchange fields
   with identical information
 */
bool Contest::vExch() const { return (_vExch); }

void Contest::setVExch(bool b) { _vExch = b; }
...
void Contest::zeroScore() {
  ...
  for (int ii = 0; ii < settings.value(c_nmulttypes, c_nmulttypes_def).toInt();
    ...
    if (multType[ii] == Uniques || multType[ii] == Special ||
        multType[ii] == Prefix) {
      mults[ii].clear();
      for (int jj = 0;
           jj < settings.value(c_nmulttypes, c_nmulttypes_def).toInt(); jj++) {
        for (int k = 0; k < NModeTypes; k++) {
          for (int j = 0; j <= N_BANDS; j++) {
