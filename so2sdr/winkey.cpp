 ...
 */
bool Winkey::winkeyIsOpen() const { return (winkeyOpen); }

/*!
   returns true if winkey is sending cw
 */
bool Winkey::isSending() const { return (sending); }
  ...
  while (winkeyPort->bytesAvailable()) {
    ...
    if (n > 0) {
      if ((wkbyte & 0xc0) == 0xc0) {
        ...
        if (wkbyte & 4) {
          if (winkeySendingCmd == true)
            continue; // ignore duplicate tx messages
          sending = true;
          ...
          // the radio may have been switched since transmit started, need rx
          // signal for the sending radio
          emit winkeyTx(false, txRig);
          QTimer::singleShot(queueDelay, this, SIGNAL(finished()));
        }
        // Pushbutton status only sent to host in WK2 mode by admin command,
        // (0x00, 11)
        ...
        // echo byte
        // in case CW was canceled, ignore any echo still associated with
        // previous message
        if (ignoreEcho) {
          ignoreEcho = false;
          return;
...
void Winkey::setEchoMode(bool b) { echoMode = b; }
...
void Winkey::openWinkey2() {
  ...
  // set sidetone config
  buff[0] =
      0x01; // Sidetone control command, next byte sets sidetone parameters
  buff[1] = 0;

  // Paddle sidetone only?  Set bit 7 (msb) of buff[1]
  if (settings.value(s_winkey_sidetonepaddle, s_winkey_sidetonepaddle_def)
          .toBool()) {
    buff[1] += 128;
  }
  ...
  buff[0] =
      0x05;     // Setup speed pot command, next three bytes setup the speed pot
  buff[1] = 10; // min wpm
...
void Winkey::closeWinkey() {
  if (!winkeyOpen || !winkeyPort->isOpen())
    return;
