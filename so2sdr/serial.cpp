...
extern "C" HAMLIB_EXPORT(int)
    write_block(hamlib_port_t *p, const char *txbuffer, size_t count);
 ...
 */
void RigSerial::hamlibModelLookup(int hamlib_nr, int &indx_mfg,
                                  int &indx_model) const {
 ...
 */
int RigSerial::hamlibNMfg() const { return (mfg.size()); }
...
void RigSerial::timerEvent(QTimerEvent *event) {
  ...
  if (settings
          ->value(s_radios_rigctld_enable[nrig],
                  s_radios_rigctld_enable_def[nrig])
          .toBool() &&
      socket->isOpen()) {
    ...
    if (event->timerId() == timerId[1]) {
      ...
      // behavior of hamlib over serial and via rigctld is different. At least
      // with hamlib/rigctld v 3.0.1, set_rit 0 does not turn off RIT like docs
      // say.
      lock.lockForWrite();
      if (clearRitFlag) {
        clearRitFlag = false;
    ...
    if (radioOK && event->timerId() == timerId[0]) {
      ...
      if (confParamsIF) {
        value_t val;
        int status =
            rig_get_ext_level(rig, RIG_VFO_CURR, confParamsIF->token, &val);
        if (status == RIG_OK) {
          // no mutex protection since ifFreq is read-only from outside class
          ifOffset =
              qRound(val.f - settings->value(s_radios_if[nrig]).toDouble());
        }
      }
    }
  }
 ...
 * @todo this does not handle digi modes correctly - there is no way to identify
 * these from hamlib
 */
...
void RigSerial::openSocket() {
  ...
  if (settings
    ...
    if (settings
            ->value(s_radios_rigctld_ip[nrig], s_radios_rigctld_ip_def[nrig])
            .toString()
            .simplified() == "localhost") {
      socket->connectToHost(QHostAddress::LocalHost,
                            settings
                                ->value(s_radios_rigctld_port[nrig],
                                        s_radios_rigctld_port_def[nrig])
                                .toInt());
    } else {
      socket->connectToHost(
          QHostAddress(settings
                           ->value(s_radios_rigctld_ip[nrig],
                                   s_radios_rigctld_ip_def[nrig])
                           .toString())
              .toString(),
          settings
              ->value(s_radios_rigctld_port[nrig],
...
void RigSerial::openRig() {
  ...
  rig_set_conf(rig, t,
               settings->value(s_radios_port[nrig], defaultSerialPort[nrig])
                   .toString()
                   .toLatin1()
                   .data());
  rig->state.rigport.parm.serial.rate =
      settings->value(s_radios_baud[nrig], s_radios_baud_def[nrig]).toInt();

  t = rig_token_lookup(rig, "ptt_type");
  token_t t_rts = rig_token_lookup(rig, "rts_state");
  token_t t_dtr = rig_token_lookup(rig, "dtr_state");

  // hamlib messes with the RTS/DTR lines when initializing a rig. If we are
  // going to use these for PTT (which is NOT done through hamlib but is in
  // so2r.cpp), first turn them off here so they don't get stuck on.
  switch (
      settings->value(s_radios_ptt_type[nrig], s_radios_ptt_type_def).toInt()) {
  case 1:
  ...
  if (rigFreq < 1.0 && nrig == 0)
    rigFreq = 14000000;
  if (rigFreq < 1.0 && nrig == 1)
    rigFreq = 7000000;
...
void RigSerial::rxSocket() {
  const QByteArray cmdNames[] = {"get_freq:", "get_mode:", "get_level: ifctr"};
  const int nCmdNames = 3;
  const QByteArray modeNames[] = {
      "USB",     "LSB", "CW",  "CWR",    "RTTY",   "RTTYR", "AM",
      "FM",      "WFM", "AMS", "PKTLSB", "PKTUSB", "PKTFM", "ECSSUSB",
      "ECSSLSB", "FAX", "SAM", "SAL",    "SAH",    "DSB"};
  const int nModeNames = 20;
  // Note: PKTLSB and PKTUSB replaced by RTTY and RTTYR. rigctld v 3.0.1 returns
  // PKT when K3 is in AFSK data mode ??!
  const rmode_t modes[] = {
      RIG_MODE_USB,   RIG_MODE_LSB,     RIG_MODE_CW,      RIG_MODE_CWR,
      RIG_MODE_RTTY,  RIG_MODE_RTTYR,   RIG_MODE_AM,      RIG_MODE_FM,
      RIG_MODE_WFM,   RIG_MODE_AMS,     RIG_MODE_RTTY,    RIG_MODE_RTTYR,
      RIG_MODE_PKTFM, RIG_MODE_ECSSUSB, RIG_MODE_ECSSLSB, RIG_MODE_FAX,
      RIG_MODE_SAM,   RIG_MODE_SAL,     RIG_MODE_SAH,     RIG_MODE_DSB};
  ...
  while (socket->bytesAvailable()) {
    ...
    for (int i = 0; i < nCmdNames; i++) {
      if (cmdList.at(0) == cmdNames[i]) {
        switch (i) {
        case 0: // get_freq. Response looks like "get_freq:;Frequency:
                // 28009360;RPRT 0"
          ...
          if (ok)
            rigFreq = f;
          break;
        case 1: // get_mode. Response looks like "get_mode:;Mode: CW;Passband:
                // 600;RPRT 0"
          ...
          break;
        case 2: // get IF center. Response looks like "get_level:
                // ifctr;8212940.000000\nRPRT 0"
          cmdList[1].truncate(14);
          iff = cmdList.at(1).toDouble(&ok);
          if (ok) {
            ifOffset =
                qRound(iff - settings->value(s_radios_if[nrig]).toDouble());
          }
          break;
        }
      }
