...
void SSBMessageDialog::initialize(QSettings *cs, QSettings *s) {
  ...
  for (int i = 0; i < sz; i++) {
    ...
    funcEditPtr[i]->setText(
        csettings->value("func", c_cq_func_def[m]).toString());
  ...
  for (int i = 0; i < sz; i++) {
    csettings->setArrayIndex(i);
    funcRecEditPtr[i]->setText(
        csettings->value("func", c_cq_rec_func_def).toString());
  ...
  for (int i = 0; i < sz; i++) {
    csettings->setArrayIndex(i);
    excFuncEditPtr[i]->setText(
        csettings->value("func", c_ex_func_def[m]).toString());
  ...
  for (int i = 0; i < sz; i++) {
    csettings->setArrayIndex(i);
    excFuncRecEditPtr[i]->setText(
        csettings->value("func", c_ex_rec_func_def).toString());
  ...
  call_rec_edit->setText(
      csettings->value(c_call_msg_rec, c_call_msg_rec_def).toString());
  call_rec_edit->setCursorPosition(0);

  sp_exc_edit->setText(
      csettings->value(c_sp_exc[m], c_sp_exc_def[m]).toString());
  sp_exc_edit->setCursorPosition(0);
  sp_exc_rec_edit->setText(
      csettings->value(c_sp_exc_rec, c_sp_exc_rec_def).toString());
  sp_exc_rec_edit->setCursorPosition(0);

  cq_exc_edit->setText(
      csettings->value(c_cq_exc[m], c_cq_exc_def[m]).toString());
  cq_exc_edit->setCursorPosition(0);
  cq_exc_rec_edit->setText(
      csettings->value(c_cq_exc_rec, c_cq_exc_rec_def).toString());
  cq_exc_rec_edit->setCursorPosition(0);

  qsl_msg_edit->setText(
      csettings->value(c_qsl_msg[m], c_qsl_msg_def[m]).toString());
  qsl_msg_edit->setCursorPosition(0);
  qsl_msg_rec_edit->setText(
      csettings->value(c_qsl_msg_rec, c_qsl_msg_rec_def).toString());
  qsl_msg_rec_edit->setCursorPosition(0);

  qsl_updated_edit->setText(
      csettings->value(c_qsl_msg_updated[m], c_qsl_msg_updated_def[m])
          .toString());
  qsl_updated_edit->setCursorPosition(0);
  qsl_updated_rec_edit->setText(
      csettings->value(c_qsl_msg_updated_rec, c_qsl_msg_updated_rec_def)
          .toString());
  qsl_updated_rec_edit->setCursorPosition(0);

  dupe_msg_edit->setText(
      csettings->value(c_dupe_msg[m], c_dupe_msg_def[m]).toString());
  dupe_msg_edit->setCursorPosition(0);
  dupe_msg_rec_edit->setText(
      csettings->value(c_dupe_msg_rec, c_dupe_msg_rec_def).toString());
  dupe_msg_rec_edit->setCursorPosition(0);

  quick_qsl_edit->setText(
      csettings->value(c_qqsl_msg[m], c_qqsl_msg_def[m]).toString());
  quick_qsl_edit->setCursorPosition(0);
  quick_qsl_rec_edit->setText(
      csettings->value(c_qqsl_msg_rec, c_qqsl_msg_rec_def).toString());
  quick_qsl_rec_edit->setCursorPosition(0);

  cancel_edit->setText(
      csettings->value(c_ssb_cancel, c_ssb_cancel_def).toString());
  cancel_edit->setCursorPosition(0);

  // audio script csettings
  // PLAY csettings
  beforePlayLineEdit1->setText(
      csettings->value(s_before_play[0], s_before_play_def).toString());
  beforePlayLineEdit1->setCursorPosition(0);
  beforePlayLineEdit2->setText(
      csettings->value(s_before_play[1], s_before_play_def).toString());
  beforePlayLineEdit2->setCursorPosition(0);
  afterPlayLineEdit1->setText(
      csettings->value(s_after_play[0], s_after_play_def).toString());
  afterPlayLineEdit1->setCursorPosition(0);
  afterPlayLineEdit2->setText(
      csettings->value(s_after_play[1], s_after_play_def).toString());
  afterPlayLineEdit2->setCursorPosition(0);
  playLineEdit1->setText(
      csettings->value(s_play_command[0], s_play_command_def).toString());
  playLineEdit1->setCursorPosition(0);
  playLineEdit2->setText(
      csettings->value(s_play_command[1], s_play_command_def).toString());
  playLineEdit2->setCursorPosition(0);
  switchRadioLineEdit1->setText(
      csettings->value(s_switch_command[0], s_switch_command_def).toString());
  switchRadioLineEdit1->setCursorPosition(0);
  switchRadioLineEdit2->setText(
      csettings->value(s_switch_command[1], s_switch_command_def).toString());
  switchRadioLineEdit2->setCursorPosition(0);

  // REC csettings
  beforeRecLineEdit->setText(
      csettings->value(s_before_rec, s_before_rec_def).toString());
  beforeRecLineEdit->setCursorPosition(0);
  afterRecLineEdit->setText(
      csettings->value(s_after_rec, s_after_rec_def).toString());
  afterRecLineEdit->setCursorPosition(0);
  recLineEdit->setText(
      csettings->value(s_rec_command, s_rec_command_def).toString());
...
//
// Audio play and record functions. These launch external scripts that record
// and play audio messages.
//
...
void SSBMessageDialog::switchRadio(int nrig) {
  ...
  scriptProcess->close();
  QStringList args =
      csettings->value(s_switch_command[nrig], s_switch_command_def)
          .toStringList();
  QString cmd = args.takeFirst();
  scriptProcess->start(cmd, args);
}

/* Begin playing an audio message. Intended to be called from the main program
 * thread
 *
...
void SSBMessageDialog::playMessage(int nrig, QString m) {
  ...
  QString temp =
      csettings->value(s_before_play[nrig], s_before_play_def).toString();
...
void SSBMessageDialog::playMessage3(int signal) {
  ...
  disconnect(scriptProcess, SIGNAL(finished(int)), nullptr, nullptr);
  QString temp =
      csettings->value(s_after_play[playMessageRig], s_after_play_def)
          .toString();
  if (!temp.simplified().isEmpty()) {
    QStringList args = QProcess::splitCommand(temp);
    QString cmd = args.takeFirst();
 ...
 * @todo need to handle case where this is called during the "before" or "after"
 * scripts
 */
