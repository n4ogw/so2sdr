...
WsjtxCallDialog::WsjtxCallDialog(QSettings &s, uiSize sizes, int nrig,
  ...
  connect(hideDupesCheckBox, SIGNAL(stateChanged(int)), this,
          SLOT(hideDupesChanged(int)));
...
void WsjtxCallDialog::setNrig(int n) {
  if (n >= 0 && n <= 1) {
    ...
    hideDupesCheckBox->setChecked(
        settings.value(s_wsjtx_hide_dupes[n], s_wsjtx_hide_dupes_def).toBool());
  }
}

void WsjtxCallDialog::setFreq(double f) { reader->setFreq(f); }

void WsjtxCallDialog::enable(bool b) { reader->enable(b); }
