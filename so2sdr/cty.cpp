...
/*! After calling constructor, must call initialize(), which reads cty file and
   sets up databases.
 */
...
void Cty::initialize(double la, double lo, int ZoneType) {
  /*! CTY file format
     ...
     indicates that the country is on the DARC WAEDC list, and counts in
                          CQ-sponsored contests, but not ARRL-sponsored
     contests).
   */
  double mylat = la;
  double mylon = lo;

  QFile file(userDirectory() + "/" +
             settings.value(c_cty, c_cty_def).toString());
  int indx;
  ...
  switch (ZoneType) {
  case 0:
    zoneFileName = dataDirectory() + "/cq_zone_latlong.dat";
    break;
  case 1:
    zoneFileName = dataDirectory() + "/itu_zone_latlong.dat";
    break;
  }
  ...
  if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
    QString tmp = "ERROR: can't open file " + userDirectory() + "/" +
                  settings.value(c_cty, c_cty_def).toString();
    emit ctyError(tmp);
    return;
  }
  ...
  while (!file.atEnd()) {
    ...
    while (i1 < (buffer_a.size() - 1)) {
      ...
      if (!i0) {
        ...
        if (cqz == -1 && ituz == -1) {
          ...
          // (with default zones set above). This is mostly for weird/unexpected
          // calls
          ...
          // not both. In IARU contest for example, do not want CQ zone
          // exceptions to be kept
          delete new_call;
        } else {
          new_call->CtyIndx = indx;
    ...
    // special case: some countries (4U1U, ...) have no alias prefixes listed,
    // only exception calls. make sure to add to main pfx list!
    if (nalias == 0) {
      Pfx *new_pfx = new Pfx;
      new_pfx->zoneOverride = false;
      new_pfx->CtyIndx = indx;
  ...
  /*!
     @todo define < operator so qsort can be used sort prefix and call exception
     lists
   */
 ...
 */
QString Cty::mySunTimes() const { return mySun; }

/*!
   number of countries in database
 */
int Cty::nCountries() const { return (countryList.size()); }
...
void Cty::sunTimes(double lat, double lon, QString &sunTime) {
  ...
  double l = m + (1.916 * sin(m * M_PI / 180.0)) +
             (0.02 * sin(2.0 * m * M_PI / 180.0)) + 282.634;
  if (l < 0.0)
    l += 360.0;
  if (l > 360.0)
    l -= 360.0;
  ...
  double ch = (cos(zenith * M_PI / 180.0) - (sd * sin(lat * M_PI / 180.0))) /
              (cd * cos(lat * M_PI / 180.0));
  if (ch > 1.0) {
  ...
  cd = cos(asin(sd));
  ch = (cos(zenith * M_PI / 180.0) - (sd * sin(lat * M_PI / 180.0))) /
       (cd * cos(lat * M_PI / 180.0));
  if (ch < -1.0) {
