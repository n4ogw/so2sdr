...
int Stew::numberField() const { return -1; }
...
unsigned int Stew::rcvFieldShown() const { return 1; }

/*!
   only count qso's on 160m in score. No mults, just qso points
 */
int Stew::Score() const { return (qsoPts); }
...
bool Stew::validateExchange(Qso *qso) {
  ...
  if (exchElement.at(idx).size() != 4 || exchElement.at(idx).at(0) < 'A' ||
      exchElement.at(idx).at(0) > 'R' || exchElement.at(idx).at(1) < 'A' ||
      exchElement.at(idx).at(1) > 'R' || exchElement.at(idx).at(2) < '0' ||
      exchElement.at(idx).at(2) > '9' || exchElement.at(idx).at(3) < '0' ||
      exchElement.at(idx).at(3) > '9') {
    ok = false;
  } else {
