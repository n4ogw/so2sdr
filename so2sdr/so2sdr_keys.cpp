  ...
  case QEvent::KeyPress: {
    ...
    case Qt::Key_C: // alt-C : bring up config menu
      if (mod == Qt::AltModifier) {
        menubar->setActiveAction(menubar->actions().at(1));
        return (true);
      }
      break;
    case Qt::Key_D: // alt-D
    ...
    case Qt::Key_F:
      ...
      if (mod == Qt::AltModifier) {
        menubar->setActiveAction(menubar->actions().at(0));
        return (true);
    ...
    case Qt::Key_H: // alt-H : bring up Help menu
      if (mod == Qt::AltModifier) {
        menubar->setActiveAction(menubar->actions().at(3));
        return (true);
      }
      break;
    case Qt::Key_M: // alt-M
    ...
    case Qt::Key_W: // alt-W : bring up Windows menu
      if (mod == Qt::AltModifier) {
        menubar->setActiveAction(menubar->actions().at(2));
        return (true);
      }
      break;
    case Qt::Key_Tab:
    ...
    case Qt::Key_F1:
      if (((duelingCQMode && !duelingCQModePause) ||
           ...
           (toggleMode && !duelingCQMode)) &&
          !excMode[activeRadio] && mod != Qt::ShiftModifier &&
          mod != Qt::ControlModifier &&
          !(altDActive > 2 &&
            altDActiveRadio == activeRadio)) { // set long CQ MSG, no CW
        sendLongCQ = true;
      } else {
        if (altDActive > 2 && altDActiveRadio == activeRadio) {
          ...
          if ((autoCQMode && !excMode[autoCQRadio]) ||
              (duelingCQMode && !excMode[activeRadio])) {
            sendLongCQ = true;
          }
        }
    ...
    case Qt::Key_F2:
      if (((duelingCQMode && !duelingCQModePause) ||
          ...
          mod != Qt::ControlModifier &&
          !(altDActive > 2 &&
            altDActiveRadio == activeRadio)) { // set short CQ MSG, no CW
        sendLongCQ = false;
      } else {
        if (altDActive > 2 && altDActiveRadio == activeRadio) {
          if (activeTxRadio != activeRadio) {
...
void So2sdr::backSlash(int kbdNr) {
  ...
  if (!qso[activeRadio]->call.isEmpty() &&
      ...
      !(qso[activeRadio]->dupe &&
        csettings->value(c_dupemode, c_dupemode_def).toInt() == 0) &&
      cqMode[activeRadio] && exchangeSent[activeRadio]) {
    if (toggleMode) {
      switchTransmit(activeRadio);
    }
    int m = cat[activeTxRadio]->modeType();
    expandMacro(
        csettings->value(c_qqsl_msg[m], c_qqsl_msg_def[m]).toByteArray());
    ...
    qso[activeRadio]->time =
        QDateTime::currentDateTimeUtc(); // update time just before logging qso
    ...
    if (wsjtx[activeRadio]->isEnabled())
      wsjtx[activeRadio]->redupe();
    rateCount[ratePtr]++;
...
void So2sdr::spaceSprint(int nr) {
  // if already in SP mode and in call field, move to exch field, otherwise, do
  // nothing
  if (!cqMode[nr]) {
    if (callFocus[nr] && !lineEditCall[nr]->text().isEmpty()) {
      ...
      if (lineEditExchange[nr]->text().simplified().isEmpty()) {
        ...
        lineEditExchange[nr]->setText(
            lineEditExchange[nr]->text().simplified() + " ");
      }
    }
    return;
  }
  if (callFocus[nr]) {
    ...
    if (lineEditCall[nr]->text().length() > 3) {
      ...
      if (!qso[nr]->dupe) {
        ...
        case PhoneType:
          expandMacro(
              csettings->value(c_call_msg, c_call_msg_def).toByteArray());
          break;
        }
        callSent[nr] = true;
      }
...
/*!
   ...
   - level==3  : log qso, switch radios. Handle with normal enter +
   switchRadios()
 */
{
  if (twokeyboard)
  ...
  case 1:
    ...
    qso[activeRadio]->freq = cat[activeRadio]->getRigFreq();
    addSpot(qso[activeRadio]->call, qso[activeRadio]->freq,
            qso[activeRadio]->dupe);
...
void So2sdr::enter(Qt::KeyboardModifiers mod, int kbdNr) {
  ...
  if (callSent[activeRadio]) {
    if (settings->value(s_settings_exchangelogs, s_settings_exchangelogs_def)
            .toBool()) {
  ...
  // change radios
  /*!
     @todo make actions below separate functions; some are duplicated elsewhere
     (backslash,...)
   */
  if (enterState[i1][i2][i3][i4] & 16384) {
    if (activeR2CQ) {
      ...
      if (cat[activeRadio]->band() != BAND_NONE)
        removeSpotFreq(cat[activeRadio]->getRigFreq(),
                       cat[activeRadio]->band());
  ...
  if (enterState[i1][i2][i3][i4] & 1) {
    if (mod != Qt::ShiftModifier && !qso[activeRadio]->dupe) {
  ...
  if (enterState[i1][i2][i3][i4] & 2) {
    int m = cat[activeTxRadio]->modeType();
    if (lineEditCall[activeRadio]->text().contains("?") &&
        (m == CWType || m == DigiType)) {
      ...
      if (autoSend && !autoSendPause &&
          (settings->value(s_settings_autosend_mode,
                           s_settings_autosend_mode_def)
                   .toInt() == 0 ||
        ...
        if (mod != Qt::ShiftModifier) {
          if (qso[activeRadio]->dupe &&
              csettings->value(c_dupemode, c_dupemode_def).toInt() ==
                  STRICT_DUPES) {
            expandMacro(csettings->value(c_dupe_msg[m], c_dupe_msg_def[m])
                            .toByteArray());
          } else {
            expandMacro(
                csettings->value(c_cq_exc[m], c_cq_exc_def[m]).toByteArray());
          }
        }
        exchangeSent[activeRadio] = true;
        callSent[activeRadio] =
  ...
  if ((enterState[i1][i2][i3][i4] & 4) && (!autoSendTrigger || autoSendPause) &&
    ...
    if (lineEditExchange[activeRadio]->text().simplified().isEmpty()) {
      ...
      if (log->contestType() != Sweepstakes_t) {
        lineEditExchange[activeRadio]->setText(
            lineEditExchange[activeRadio]->text().simplified() + " ");
      }
    }
  }
  ...
  if (enterState[i1][i2][i3][i4] & 512) {
    ...
    if (qso[activeRadio]->call != origCallEntered[activeRadio]) {
      if (mod != Qt::ShiftModifier)
        expandMacro(
            csettings->value(c_qsl_msg_updated[m], c_qsl_msg_updated_def[m])
                .toByteArray());
    } else {
      if (mod != Qt::ShiftModifier)
        expandMacro(
            csettings->value(c_qsl_msg[m], c_qsl_msg_def[m]).toByteArray());
    }
    exchangeSent[activeRadio] = false;
  }
  ...
  if (enterState[i1][i2][i3][i4] & 8) {
    ...
    if (!cqMode[activeRadio]) {
      ...
      qso[activeRadio]->freq = cat[activeRadio]->getRigFreq();
      addSpot(qso[activeRadio]->call, qso[activeRadio]->freq, true);
      spotListPopUp[activeRadio] =
          true; // this prevents the call from immediately popping up
    } else {
    ...
    editingExchange[activeRadio] = false;
    if (mod != Qt::ShiftModifier) { // don't unpause autoCQ or dueling CQ if
                                    // silently logged
      if (autoCQMode) {
      ...
      if (duelingCQMode) {
        duelingCQWait = true; // prevent premature dueling CQ takeover until QSL
                              // message is complete
  ...
  if (enterState[i1][i2][i3][i4] & 32 ||
      lineEditCall[activeRadio]->text().contains("?")) {
    callFocus[activeRadio] = true;
    setEntryFocus(activeRadio);
  }
  ...
  if (enterState[i1][i2][i3][i4] & 256) {
    if (mod != Qt::ShiftModifier) {
      if (activeR2CQ && csettings->value(c_sprintmode, c_sprintmode_def)
                            .toBool()) { // sprint 2nd radio CQ
        expandMacro("{R2CQ}" + cwMessage->cqF[0]);
      } else {
        switch (cat[activeTxRadio]->modeType()) {
        case CWType:
        case DigiType:
          if ((sendLongCQ && (toggleMode || autoCQMode)) ||
              !(toggleMode || autoCQMode)) {
  ...
  if (enterState[i1][i2][i3][i4] & 32768) {
    if (!csettings->value(c_sprintmode, c_sprintmode_def)
             .toBool()) { // not sprint
  ...
  if (mod != Qt::AltModifier) {
      ...
      if ((!twokeyboard && cw->isSending()) ||
        ...
        if (autoCQMode &&
            ((!altDActive && lineEditCall[activeRadio]->text().isEmpty()) ||
             (altDActive &&
              lineEditCall[altDActiveRadio ^ 1]->text().isEmpty()))) {
          autoCQModePause = true;
        }
        if (toggleMode && !duelingCQMode)
          toggleStatus->setText("<font color=#0000FF>TOGGLE</font>");
    ...
    case PhoneType: {
      if ((!twokeyboard && ssbMessage->isPlaying()) ||
        ...
        expandMacro(
            csettings->value(c_ssb_cancel, c_ssb_cancel_def).toByteArray());
      }
      break;
    }
    case DigiType:
...
void So2sdr::sendFunc(int i, Qt::KeyboardModifiers mod) {
  ...
  switch (mod) {
  case Qt::NoModifier:
    if (i == 1 && csettings->value(c_sprintmode, c_sprintmode_def).toBool() &&
        cqQsoInProgress[activeRadio]) {
      /*!
    ...
    if (kbdHandler[0] && kbdHandler[1]) {
      ...
      kbdHandler[0]->setDevice(
          settings->value(s_twokeyboard_device[0], s_twokeyboard_device_def[0])
              .toString());
      kbdHandler[1]->setDevice(
          settings->value(s_twokeyboard_device[1], s_twokeyboard_device_def[1])
              .toString());
    } else {
      for (int i = 0; i < NRIG; i++) {
        if (!kbdHandler[i]) {
          kbdHandler[i] = new KeyboardHandler(
              settings
                  ->value(s_twokeyboard_device[i], s_twokeyboard_device_def[i])
                  .toString());
        }
      }
      connect(kbdHandler[0], SIGNAL(readKey(int, bool, bool, bool)), this,
              SLOT(kbd1(int, bool, bool, bool)));
      connect(kbdHandler[1], SIGNAL(readKey(int, bool, bool, bool)), this,
              SLOT(kbd2(int, bool, bool, bool)));
      for (int i = 0; i < NRIG; i++) {
...
/*!
 * \brief So2sdr::handleKeys feeds keypresses in two keyboard mode into Qt event
 * system \param nr : keyboard number (0 or 1) \param code : keycodev \param
 * shift, ctrl, alt : indicate presence of key modifiers
 */
void So2sdr::handleKeys(int nr, int code, bool shift, bool ctrl, bool alt) {
  if (code < NKEYS) {
    ...
    if (errorBox->isVisible() || cabrillo->isVisible() ||
        options->isVisible() || cwMessage->isVisible() ||
        ssbMessage->isVisible() || cwDialog->isVisible() || sdr->isVisible() ||
