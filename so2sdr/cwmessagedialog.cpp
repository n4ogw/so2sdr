...
CWMessageDialog::CWMessageDialog(ModeTypes modetype, uiSize sizes,
  ...
  m = (int)modetype;
  if (m > 1)
    m = 0; // digi keys not implemented yet
...
void CWMessageDialog::initialize(QSettings *s) {
  ...
  for (int i = 0; i < sz; i++) {
    ...
    funcEditPtr[i]->setText(
        settings->value("func", c_cq_func_def[m]).toString());
  ...
  for (int i = 0; i < sz; i++) {
    ...
    excFuncEditPtr[i]->setText(
        settings->value("func", c_ex_func_def[m]).toString());
  ...
  for (int i = 0; i < sz; i++) {
    settings->setArrayIndex(i);
    shiftFuncEditPtr[i]->setText(
        settings->value("func", c_shift_func_def[m]).toString());
  ...
  for (int i = 0; i < sz; i++) {
    settings->setArrayIndex(i);
    ctrlFuncEditPtr[i]->setText(
        settings->value("func", c_ctrl_func_def[m]).toString());
  ...
  sp_exc_edit->setText(
      settings->value(c_sp_exc[m], c_sp_exc_def[m]).toString());
  sp_exc_edit->setCursorPosition(0);
  cq_exc_edit->setText(
      settings->value(c_cq_exc[m], c_cq_exc_def[m]).toString());
  cq_exc_edit->setCursorPosition(0);
  qsl_msg_edit->setText(
      settings->value(c_qsl_msg[m], c_qsl_msg_def[m]).toString());
  qsl_msg_edit->setCursorPosition(0);
  qsl_updated_edit->setText(
      settings->value(c_qsl_msg_updated[m], c_qsl_msg_updated_def[m])
          .toString());
  qsl_updated_edit->setCursorPosition(0);
  dupe_msg_edit->setText(
      settings->value(c_dupe_msg[m], c_dupe_msg_def[m]).toString());
  dupe_msg_edit->setCursorPosition(0);
  quick_qsl_edit->setText(
      settings->value(c_qqsl_msg[m], c_qqsl_msg_def[m]).toString());
...
CWMessageDialog::~CWMessageDialog() { delete upperValidate; }
