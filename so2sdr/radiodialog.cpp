...
RadioDialog::RadioDialog(QSettings &s, RigSerial &cat, uiSize sizes,
  ...
  connect(checkBoxRigctld1, SIGNAL(toggled(bool)), this,
          SLOT(rigctld1Checked(bool)));
  connect(checkBoxRigctld2, SIGNAL(toggled(bool)), this,
          SLOT(rigctld2Checked(bool)));
  ...
  connect(radioMfgComboBox[0], SIGNAL(currentIndexChanged(int)), this,
          SLOT(populateModels1(int)));
  connect(radioMfgComboBox[1], SIGNAL(currentIndexChanged(int)), this,
          SLOT(populateModels2(int)));
...
void RadioDialog::populateModels1(int indx) { populateModelCombo(0, indx); }

void RadioDialog::populateModels2(int indx) { populateModelCombo(1, indx); }
...
void RadioDialog::populateModelCombo(int nrig, int mfg_indx) {
  ...
  for (int i = 0; i < catptr.hamlibNModels(mfg_indx); i++) {
    radioModelComboBox[nrig]->insertItem(i,
                                         catptr.hamlibModelName(mfg_indx, i));
  }
}
...
void RadioDialog::updateRadio() {
  for (int i = 0; i < NRIG; i++) {
    ...
    settings.setValue(
        s_radios_rig[i],
        catptr.hamlibModelIndex(radioMfgComboBox[i]->currentIndex(),
                                radioModelComboBox[i]->currentIndex()));
    settings.setValue(s_radios_ptt_type[i],
                      radioPttComboBox[i]->currentIndex());
...
void RadioDialog::updateFromSettings() {
  checkBoxRigctld1->setChecked(
      settings.value(s_radios_rigctld_enable[0], s_radios_rigctld_enable_def[0])
          .toBool());
  checkBoxRigctld2->setChecked(
      settings.value(s_radios_rigctld_enable[1], s_radios_rigctld_enable_def[1])
          .toBool());
  rigctld1Checked(
      settings.value(s_radios_rigctld_enable[0], s_radios_rigctld_enable_def[0])
          .toBool());
  rigctld2Checked(
      settings.value(s_radios_rigctld_enable[1], s_radios_rigctld_enable_def[1])
          .toBool());
  lineEditIp1->setText(
      settings.value(s_radios_rigctld_ip[0], s_radios_rigctld_ip_def[0])
          .toString());
  lineEditIp2->setText(
      settings.value(s_radios_rigctld_ip[1], s_radios_rigctld_ip_def[1])
          .toString());
  lineEditPort1->setText(
      settings.value(s_radios_rigctld_port[0], s_radios_rigctld_port_def[0])
          .toString());
  lineEditPort2->setText(
      settings.value(s_radios_rigctld_port[1], s_radios_rigctld_port_def[1])
          .toString());

  for (int i = 0; i < NRIG; i++) {
    radioDevEdit[i]->setText(
        settings.value(s_radios_port[i], defaultSerialPort[i]).toString());
    radioPollTimeEdit[i]->setText(
        settings.value(s_radios_poll[i], s_radios_poll_def[i]).toString());
    int indx1;
    int indx2;
    catptr.hamlibModelLookup(
        settings.value(s_radios_rig[i], s_radios_rig_def[i]).toInt(), indx1,
        indx2);
    radioMfgComboBox[i]->setCurrentIndex(indx1);
    radioModelComboBox[i]->setCurrentIndex(indx2);

    radioDevEdit[i]->setText(
        settings.value(s_radios_port[i], s_radios_port_def[i]).toString());
    radioPollTimeEdit[i]->setText(
        settings.value(s_radios_poll[i], s_radios_poll_def[i]).toString());
    radioIFEdit[i]->setText(
        settings.value(s_radios_if[i], s_radios_if_def[i]).toString());
    ...
    radioPttComboBox[i]->setCurrentIndex(
        settings.value(s_radios_ptt_type[i], s_radios_ptt_type_def).toInt());
  }
}
