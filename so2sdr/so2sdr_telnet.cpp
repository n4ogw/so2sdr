...
void So2sdr::loadSpots() {
  ...
  while (!spotFile->atEnd()) {
    ...
    if (b >= 0 && b < N_BANDS) {
      ...
      s >> newSpot->f;
      if ((newSpot->createdTime +
           settings->value(s_sdr_spottime, s_sdr_spottime_def).toInt() * 1000) >
          currentTime) {
...
/*!
   checks spot list to see if freq is close to a spot. If so, pop up call to
   window

   spotListPopUp prevents the same spot from being popped up multiple times
   unless the frequency has changed.
...
void So2sdr::checkSpot(int nr) {
  ...
  if (found) {
    ...
    if (cat[nr]->modeType() == CWType && cw->isSending() &&
        nr == activeTxRadio) {
      cw->cancelcw();
    }
    if (cat[nr]->modeType() == PhoneType && ssbMessage->isPlaying() &&
        nr == activeTxRadio) {
    ...
    if (settings->value(s_settings_qsyfocus, s_settings_qsyfocus_def)
            .toBool()) {
      if (lineEditCall[nr ^ 1]->text().simplified().isEmpty() &&
          lineEditExchange[nr ^ 1]->text().simplified().isEmpty() &&
          nr != activeRadio && !activeR2CQ && !cw->isSending() &&
        ...
        if (altDActive && nr != altDActiveRadio &&
            lineEditCall[altDActiveRadio]->text().simplified().isEmpty()) {
          altDActive = false;
          if (altDOrigMode &&
              !csettings->value(c_sprintmode, c_sprintmode_def).toBool()) {
