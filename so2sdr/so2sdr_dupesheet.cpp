...
/* the program operates differently depending on whether one or two dupesheets
 *are open:
 *
 *1) If only one dupesheet is open, it follows the active radio. This is the
 *correct behavior for Sprint operating.
 *
 *2) If two dupesheets are open, one shows each radio.
 *
 *If you have two open and then close one, the program switches over to the
 *other mode. Similar behavior if one is open and then a second is opened.
 **/

void So2sdr::initDupeSheet() {
  connect(dupesheetAction1, SIGNAL(triggered(bool)), this,
          SLOT(showDupesheet1(bool)));
  connect(dupesheetAction2, SIGNAL(triggered(bool)), this,
          SLOT(showDupesheet2(bool)));
  for (int nr = 0; nr < NRIG; nr++) {
    ...
    settings->endGroup();
    dupesheet[nr]->setWindowFlags(
        Qt::Window | Qt::WindowTitleHint | Qt::CustomizeWindowHint |
        Qt::WindowCloseButtonHint | Qt::WindowMinimizeButtonHint);
  }
  connect(dupesheet[0], SIGNAL(closed(bool)), dupesheetAction1,
          SLOT(setChecked(bool)));
  connect(dupesheet[1], SIGNAL(closed(bool)), dupesheetAction2,
          SLOT(setChecked(bool)));
}
...
int So2sdr::nDupesheet() const {
  int i = 0;
  if (dupesheet[0]->isVisible())
    i++;
  if (dupesheet[1]->isVisible())
    i++;
  return i;
}
...
void So2sdr::populateDupesheet() {
  ...
  for (int id = 0; id < NRIG; id++) {
    ...
    int b = cat[ib]->band();
    if (b == BAND_NONE || b == dupesheet[id]->band())
      continue;

    dupesheet[id]->clear();
    dupesheet[id]->setBand(b);
    QSqlQueryModel m;
    m.setQuery("SELECT * FROM log WHERE valid=1 and BAND=" +
               QString::number(b));
