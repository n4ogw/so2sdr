...
Cwdaemon::Cwdaemon(QSettings &s, QObject *parent) : QObject(parent), settings(s)
{
    sendBuff.clear();
    sent.clear();
    sending        = false;
 ...
 */
bool Cwdaemon::isOpen(int nrig) const
{
    return(open_[nrig]);
}
...
void Cwdaemon::sendcw()
{
    ...
    socket->writeDatagram(buff,2,QHostAddress::LocalHost,settings.value(s_cwdaemon_udp[rigNum],s_cwdaemon_udp_def[rigNum]).toInt());
    socket->writeDatagram(sendBuff.data(),sendBuff.size(),QHostAddress::LocalHost,settings.value(s_cwdaemon_udp[rigNum],s_cwdaemon_udp_def[rigNum]).toInt());
    sendBuff.clear();
    sending=true;
    txRig=rigNum;
    emit(tx(true,rigNum));
...
void Cwdaemon::setUdpPort(int nrig,int p)
{
    udpPort[nrig] = p;
}
...
{
    if (!open_[rigNum]) return;

    char cmd[2]={0x1b,0x34};
    socket->writeDatagram(cmd,2,QHostAddress::LocalHost,settings.value(s_cwdaemon_udp[rigNum],s_cwdaemon_udp_def[rigNum]).toInt());
    emit(cwCanceled());
}
...
void Cwdaemon::setSpeed(int speed)
{
    ...
    socket->writeDatagram(s,s.size(),QHostAddress::LocalHost,settings.value(s_cwdaemon_udp[rigNum],s_cwdaemon_udp_def[rigNum]).toInt());
}

/*!
   choose radio to send on
 */
void Cwdaemon::switchTransmit(int nrig)
{
    rigNum=nrig;
}
...
void Cwdaemon::open()
{
    ...
    for (int i=0;i<NRIG;i++) {
        const char buff[2]={0x1b,0x68}; // <ESC> h
        sendBuff=" ";
        socket[i].writeDatagram(buff,2,QHostAddress::LocalHost,settings.value(s_cwdaemon_udp[i],s_cwdaemon_udp_def[i]).toInt());
        socket[i].writeDatagram(sendBuff.data(),sendBuff.size(),QHostAddress::LocalHost,
