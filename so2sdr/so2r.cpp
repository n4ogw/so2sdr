...
{
    if (settings.value(s_radios_ptt_type[nr],s_radios_ptt_type_def).toInt()==0) return;
    int fd=open(settings.value(s_radios_port[nr],defaultSerialPort[nr]).toByteArray().data(),O_RDWR | O_NOCTTY | O_NONBLOCK);
    int flag;
    if (settings.value(s_radios_ptt_type[nr],s_radios_ptt_type_def).toInt()==1) {
        flag=TIOCM_RTS;
    } else {
        flag=TIOCM_DTR;
    }
...
So2r::So2r(QSettings &s, QObject *parent) : QObject(parent),settings(s)
{
    ...
    so2rDialog=new So2rDialog(settings);
    so2rDialog->hide();
    ...
    connect(otrsp[0], SIGNAL(otrspError(const QString &)), this,SIGNAL(error(const QString &)));
    connect(otrsp[1], SIGNAL(otrspError(const QString &)), this,SIGNAL(error(const QString &)));
    connect(microham, SIGNAL(microhamError(const QString &)), this,SIGNAL(error(const QString &)));
    connect(pport, SIGNAL(parallelPortError(const QString &)), this,SIGNAL(error(const QString &)));
    connect(otrsp[0], SIGNAL(otrspNameSet(QByteArray,int)), so2rDialog, SLOT(setOtrspName(QByteArray,int)));
    connect(otrsp[1], SIGNAL(otrspNameSet(QByteArray,int)), so2rDialog, SLOT(setOtrspName(QByteArray,int)));
...
{
    return (pport->stereoActive() || otrsp[0]->stereoActive() || otrsp[1]->stereoActive() || microham->stereoActive());
}

void So2r::switchAudio(int r)
{
    if (r<0 || r>=NRIG) return;
