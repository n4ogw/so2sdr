...
bool BandmapInterface::bandmapon(int nr) const { return bandmapOn[nr]; }
...
BandmapInterface::BandmapInterface(QSettings &s, QObject *parent)
  ...
  connect(&socket[1], SIGNAL(stateChanged(QAbstractSocket::SocketState)), this,
          SLOT(launchTcpSocketStateChange2(QAbstractSocket::SocketState)));
}
...
void BandmapInterface::launchTcpSocketStateChange1(
    QAbstractSocket::SocketState state) {
  tcpSocketStateChange(0, state);
}

void BandmapInterface::launchTcpSocketStateChange2(
    QAbstractSocket::SocketState state) {
  tcpSocketStateChange(1, state);
}

void BandmapInterface::tcpSocketStateChange(
    int nr, QAbstractSocket::SocketState state) {
...
void BandmapInterface::launchShowBandmapProcessError1(
    QProcess::ProcessError err) {
  showBandmapProcessError(0, err);
}

/*! QProcess error connection for bandmap 2 */
void BandmapInterface::launchShowBandmapProcessError2(
    QProcess::ProcessError err) {
  showBandmapProcessError(1, err);
}

/*! Shows QProcess errors */
void BandmapInterface::showBandmapProcessError(int nr,
                                               QProcess::ProcessError err) {
  Q_UNUSED(err)
  Q_UNUSED(nr)
}
...
void BandmapInterface::addSpot(int nr, const BandmapEntry &spot) {
  if (bandmapOn[nr] && socket[nr].state() == QAbstractSocket::ConnectedState) {
    ...
    str = str + "," + QByteArray::number(spot.f, 'f', 0) + ",";
    if (spot.dupe) {
      str = str + char(0xff) + char(0x00) + char(0xff) + char(0xff) +
            char(0x00) + char(0xff);
      str = str + char(0x01);
    } else {
      str = str + char(0x00) + char(0x00) + char(0x00) + char(0xff) +
            char(0xff) + char(0xff);
      str = str + char(0x00);
    }
    char len = str.length();
...
void BandmapInterface::nextFreq(int nr, bool higher) {
  if (bandmapOn[nr] && socket[nr].state() == QAbstractSocket::ConnectedState) {
    char cmd;
    if (higher)
      cmd = BANDMAP_CMD_QSY_UP;
    else
      cmd = BANDMAP_CMD_QSY_DOWN;
    const char len = 0;
...
void BandmapInterface::connectTcp() {
  for (int i = 0; i < NRIG; i++) {
    if (!bandmapOn[i]) {
      if (bandmapAvailable[i] &&
          socket[i].state() == QAbstractSocket::UnconnectedState) {
        socket[i].connectToHost(
            settings.value(s_sdr_ip[i], s_sdr_ip_def[i]).toString(),
...
void BandmapInterface::setAddOffset(double f, int nr) {
  ...
  if (bandmapOn[nr] && socket[nr].state() == QAbstractSocket::ConnectedState) {
    ...
    QByteArray str = QByteArray::number(f, 'f', 0);
    char len = str.length();
...
/*!
 * \brief BandmapInterface::bandmapSetFreq Send command to bandmap setting
 * center frequency \param f frequency (Hz) \param nr radio number (0,1)
 */
 ...
 *
 */
int BandmapInterface::currentBand(int nr) const { return band[nr]; }
