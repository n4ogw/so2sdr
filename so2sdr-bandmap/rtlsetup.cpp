 ...
 * \param parent
 */
RtlSetup::RtlSetup(QSettings &s,uiSize sizes,QWidget *parent) : QDialog(parent),settings(s)
{
    setupUi(this);
...
{
    if (band==BAND_NONE) return settings.value(s_sdr_offset_rtl,s_sdr_offset_rtl_def).toDouble();
...
{
    if (band==BAND_NONE) return settings.value(s_sdr_swap_rtl,s_sdr_swap_rtl_def).toBool();
...
void RtlSetup::updateFromSettings()
{
    ...
    lineEditOffset->setText(settings.value(s_sdr_offset_rtl,s_sdr_offset_rtl_def).toString());
    checkBoxSwap->setChecked(settings.value(s_sdr_swap_rtl,s_sdr_swap_rtl_def).toBool());
    deviceIndexLineEdit->setText(settings.value(s_sdr_rtl_dev_index,s_sdr_rtl_dev_index_def).toString());
    tunerGainLineEdit->setText(settings.value(s_sdr_rtl_tuner_gain,s_sdr_rtl_tuner_gain_def).toString());
    IFFreqLineEdit->setText(settings.value(s_sdr_rtl_if_freq,s_sdr_rtl_if_freq_def).toString());
    directCheckBox->setChecked(settings.value(s_sdr_rtl_direct,s_sdr_rtl_direct_def).toBool());
