...
void Spectrum::updateParams() {
  // make local copies of settings objects; accessing settings object in
  // processdata slows bandmap considerably
...
/*! setting calcErrorNext=true will trigger a IQ error calculation
   on the next sweep */
void Spectrum::setCalcError() { calcErrorNext = true; }

/*! select plotting of IQ phase/gain points
 */
void Spectrum::setPlotPoints(bool b) { iqPlotOpen = b; }
...
void Spectrum::findCQ(double flow, double fhigh, QList<Call> &callList) {
  ...
  Signal *sigListCQtmp;
  int totSize = SIG_MAX;
  if (settings.value(s_sdr_cq_finder_calls, s_sdr_cq_finder_calls_def)
          .toBool()) {
  ...
  // adjust ends
  if (findCQLimit[0] < endFreqs[0])
    findCQLimit[0] = endFreqs[0];
  if (findCQLimit[1] > endFreqs[1])
    findCQLimit[1] = endFreqs[1];
 ...
 */
void Spectrum::stopSpectrum() { saveError(); }
...
void Spectrum::processData(unsigned char *data, unsigned int bptr) {
  ...
  for (int i = 0; i < fftSize; i++) {
    ...
    case 1: // 24 bit
    {
      // convert 24 bit signed integer sample to floating point by placing data
      // into 32-bit signed int data is in ptr[0](LSB) ... ptr[2](MSB)
      //
  ...
  if (iqCorrect) {
    for (int i = 0; i < fftSize; i++) {
      ...
      double real = out[i][0] + out[j][0] -
                    (out[i][1] + out[j][1]) * errfunc[i][1] +
                    (out[i][0] - out[j][0]) * errfunc[i][0];
      double imag = out[i][1] - out[j][1] +
                    (out[i][1] + out[j][1]) * errfunc[i][0] +
                    (out[i][0] - out[j][0]) * errfunc[i][1];
  ...
  if (scale == 2) {
    ...
    double tmp =
        (offsetSign * (offset + addOffset) * fftSize * scale) / sampleFreq;
...
void Spectrum::setAddOffset(double f) { addOffset = f; }
...
void Spectrum::detectPeaks(double bg, double sigma, double spec[]) {
  ...
  spec_smooth[fftSize - 2] =
      1.0 / 3.0 * (spec[fftSize - 1] + spec[fftSize - 2] + spec[fftSize - 3]);
  ...
  for (int i = 2; i < (fftSize - 2); i++) {
    peakAvg[peakAvgCnt][i] =
        1.0 / 5.0 *
        (spec[i] + spec[i - 1] + spec[i + 1] + spec[i + 2] + spec[i - 2]);
  }
  ...
  double cut =
      bg + settings.value(s_sdr_level, s_sdr_level_def).toInt() * sigma;
  ...
  while (i < fftSize) {
    if (spec_smooth[i] > cut) {
      ...
      int j = i + 1;
      while (j < fftSize) {
        if (spec_smooth[j] < cut)
          break;
        j++;
      }

      // not a good peak
      if (j == fftSize || ((j - i) < SIG_PEAK_MIN_WIDTH) ||
        ...
        if (ipk > fftSize / 2) {
          // neg freqs
          freq = centerFreq - invert * totOffset -
                 invert * qRound((fftSize - ipk) * sampleFreq /
                                 static_cast<double>(fftSize));
        } else {
          // positive freqs
          freq =
              centerFreq - invert * totOffset +
              invert * qRound(ipk * sampleFreq / static_cast<double>(fftSize));
        }
            ...
            if (sigList[j].cnt > 5) {
              ...
              for (int k = 0; k < SIG_MAX; k++) {
                ...
                if (sigListCQ[k].active &&
                    abs(sigListCQ[k].f - freq) < SIG_MIN_FREQ_DIFF) {
                  indx2 = k;
                  sigListCQ[k].f = freq;
              ...
              if (indx2 == -1) {
                ...
                if (indx2 != -1) {
                  ...
                  sigListCQ[indx2].n =
                      static_cast<int>(findCQTime / 0.008 / n_avg);
                }
              }
            }
            break;
 ...
 */
void Spectrum::resetAvg() { peakAvgCnt = 0; }
...
/*! calculate gain and phase error for IQ balancing
   ...
   if force=true, will fit errors regardless of status of "measure data" option.
   This is needed to fit the error read in from disk when the bandmaps starts.
   Otherwise, fits only need to be done if data is being collected.
 */
void Spectrum::calcError(bool force) {
  if (!force && !settings.value(s_sdr_iqdata, s_sdr_iqdata_def).toBool())
    return;
  double t;
...
bool Spectrum::readError() {
  ...
  QByteArray fname =
      "iq" +
      QByteArray::number(settings.value(s_sdr_nrig, s_sdr_nrig_def).toInt()) +
      ".dat";
  QFile file(fname);
 ...
 */
void Spectrum::measureBackground(double &background, double &sigma,
                                 double spec[]) const {
 ...
 */
void Spectrum::measureBackgroundLog(double &background, double &sigma,
                                    double spec[]) const {
  // estimate average and standard error; only keep every 4th point
  double avg = 0.;
  for (int i = 0; i < fftSize; i += 4) {
...
void Spectrum::makeWindow() {
  ...
  for (int i = 0; i < fftSize; i++) {
    window[i] = a0 - a1 * cos(i * tmp) + a2 * cos(2.0 * i * tmp) -
                a3 * cos(3.0 * tmp * i);
  }
}
 ...
 */
void Spectrum::gaussElim(double a[FIT_ORDER][FIT_ORDER], double y[FIT_ORDER],
                         int n) {
  double t;
  for (int i = 0; i < n; i++) {
    int max = i;
...
void Spectrum::setTuning(bool b) { isTuning = b; }

void Spectrum::setPeakDetect(bool b) { peakDetect = b; }
