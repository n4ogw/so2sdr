...
void BandOffsetSetup::updateFromSettings()
{
    ...
    for (int i=0;i<sz;i++) {
        ...
        QList<QStandardItem*> itemList;
        itemList.clear();
        QStandardItem *item1=new QStandardItem(bandName[settings.value("band",0).toInt()]);
        itemList.append(item1);
        QStandardItem *item2=new QStandardItem(settings.value("offset",0).toString());
        itemList.append(item2);
        QStandardItem *item3=new QStandardItem("");
        item3->setCheckable(true);
...
void BandOffsetSetup::updateSettings()
{
    ...
    for (int i=0;i<model->rowCount();i++) {
        settings.setArrayIndex(i);
        settings.setValue("band",delegate->bandNameToIndex(model->item(i,0)->data(Qt::DisplayRole).toString()));
        settings.setValue("offset",model->item(i,1)->data(Qt::DisplayRole).toInt());
        if (model->item(i,2)->checkState()==Qt::Checked)
...
QWidget* ComboBoxItemDelegate::createEditor(QWidget* parent, const QStyleOptionViewItem& option, const QModelIndex& index) const
{
    ...
    for (int i=0;i<N_BANDS;i++) box->addItem(bandName[i]);
    return box;
}
