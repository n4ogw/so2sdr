...
void AudioReaderPortAudio::initialize() {
  ...
  inputParameters.device =
      settings->value(s_sdr_deviceindx, s_sdr_deviceindx_def).toInt();
  ...
  err = Pa_OpenStream(
      ...
      settings->value(s_sdr_sound_sample_freq, s_sdr_sound_sample_freq_def)
          .toInt(),
      periodSize, paClipOff | paDitherOff, callback, this);
  if (checkError(err)) {
 ...
 */
void AudioReaderPortAudio::stop() { stopFlag = true; }
 ...
 */
int AudioReaderPortAudio::callback(const void *input, void *output,
  ...
  unsigned long sz =
      static_cast<AudioReaderPortAudio *>(userdata)->sizes.advance_size;
  ...
  for (unsigned long i = 0;
       i <
       frameCount * static_cast<AudioReaderPortAudio *>(userdata)->frameSize;
       i++) {
    *ptr_out = *ptr_in;
    ptr_in++;
    ptr_out++;
  }
  static_cast<AudioReaderPortAudio *>(userdata)->ptr +=
      frameCount * static_cast<AudioReaderPortAudio *>(userdata)->frameSize;
  static_cast<AudioReaderPortAudio *>(userdata)->iptr +=
      frameCount * static_cast<AudioReaderPortAudio *>(userdata)->frameSize;

  // make buffer circular
  if (static_cast<AudioReaderPortAudio *>(userdata)->iptr ==
      static_cast<AudioReaderPortAudio *>(userdata)->sizes.chunk_size) {
    static_cast<AudioReaderPortAudio *>(userdata)->iptr = 0;
    static_cast<AudioReaderPortAudio *>(userdata)->ptr =
        static_cast<AudioReaderPortAudio *>(userdata)->buff;
  }
...
unsigned int AudioReaderPortAudio::sampleRate() const
{
    return settings->value(s_sdr_sound_sample_freq, s_sdr_sound_sample_freq_def).toUInt();
}
